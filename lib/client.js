window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function loadAllEvents(connection, sessionId) {
			...
			let beforeSeq;
			for (;;) {
				const page = await connection.api.sessions.history({
					sessionId,
					beforeSeq,
					maxMessages: 200
				});
		...
		//#region component
		/**
		 * The composer-dock cell: reads the session's token usage and event log,
		 * prices it with the built-in rate table, and renders the running total.
		 */
		function BillingDock({ fetchEstimate, sessionId, useProjection }) {
			...
			if (failed) return react.createElement("span", { className: "dsh-billing-cell" }, "费用计算不可用");
			if (data === null) return null;
			const t = data.total;
			if (t === undefined || (t.inputTokens + t.outputTokens + t.cacheReadTokens + t.cacheWriteTokens) === 0) {
				return react.createElement("span", { className: "dsh-billing-cell" }, "费用 ¥0.00");
			}
			const per = (data.perModel || []).map((m) => `${m.model}: ¥${m.cost !== undefined ? m.cost.toFixed(4) : "?"}（入 ${fmtTokens(m.inputTokens)} · 出 ${fmtTokens(m.outputTokens)}）`).join("；");
			return react.createElement("span", {
				className: "dsh-billing-cell",
				title: per || undefined
			}, `费用 ${fmtYuan(t.cost)} · 输入 ${fmtTokens(t.inputTokens + t.cacheReadTokens + t.cacheWriteTokens)} · 输出 ${fmtTokens(t.outputTokens)}`);
		}
		//#endregion
		...
		/**
		 * Client plugin body: register the billing cell in the composer dock.
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"dsh-conversation-billing\"]") === null) {
			...
			tag.textContent = ".dsh-billing-cell{font-size:12px;line-height:1.4;color:var(--dsw-alias-label-secondary,inherit);opacity:.85;white-space:nowrap}";
